/*
 ODES_master.c                                           sudowrestler@gmail.com
...
int lenL[NUM_THREADS], product[NUM_THREADS], rLen[NUM_THREADS], 
    totNumSubGraphs[NUM_THREADS];
int **hits[NUM_THREADS], subGnodes[NUM_THREADS][MAX_SUBG_VERTS];
char **labelMap;
gintGraph *gintG, ** potentialGraphsToReturn[NUM_THREADS];
gnGraph **denseSubGraphsToReturn[NUM_THREADS];
subGraphChain *L[NUM_THREADS], *masterCurr, *sgcCurr[NUM_THREADS];

#ifndef EXCLUDED_EDGES /* experimental, set in odes.h */
...
#endif
{
  ...
  */
  ...
  pthread_create(&pt[NUM_THREADS-1], NULL, (void *(*)(void*))ThreadFunc, 
                 (void *)(ta+NUM_THREADS-1));
...
int ProcessSubGraph(int idx, gintGraph *subG, int *limbs, int minNodes,
                    double desiredDensity)
{
  int i, j, k, canBeExtended, existingEdges, insertionIndex, numNewEdges;
  void *vp;
  double den;
  register int tmp;
  gintNode *node;
    ...
    */
    ...
    if(den >= desiredDensity)
    {
      ...
      */
      ...
      /* it's not there, so add to buffer L[(idx+1)%NUM_THREADS] */
      lenL[idx]++;
      product[idx] = 1;
      if(lenL[idx] > rLen[idx])
      {
        rLen[idx] += REALLOCLEN;
        hits[idx] = (int **) realloc(hits[idx], rLen[idx]*sizeof(int *));
        if(hits[idx]==NULL)
        {
      ...
      /* place sgcPt in thread[(idx+1)%NUM_THREADS] buffer */
